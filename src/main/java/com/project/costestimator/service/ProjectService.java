...
public class ProjectService {
    ...
    public ProjectDetail update(UUID id, ProjectRequest request) { validateDates(request.plannedStartDate(), request.plannedEndDate()); var p = requireProject(id); apply(p, request); return detail(p); }
    ...
    private void apply(Project p, ProjectRequest r) { p.setCode(r.code()); p.setName(r.name()); p.setDescription(r.description()); p.setPlannedStartDate(r.plannedStartDate()); p.setPlannedEndDate(r.plannedEndDate()); p.setCurrency(Currency.getInstance(r.currencyCode().toUpperCase())); p.setStatus(r.status() == null ? ProjectStatus.DRAFT : r.status()); }
    ...
    private ProjectSummary summary(Project p) { return new ProjectSummary(p.getId(), p.getCode(), p.getName(), p.getPlannedStartDate(), p.getPlannedEndDate(), p.getCurrency(), p.getStatus()); }
